...
const Volcanoes = () => {
  const [volcanoesPosts, setVolcanoesPosts] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchVolcanoesPosts = async () => {
      try {
        const response = await fetch("http://localhost:5000/api/blogposts/category/Volcanoes");
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        setVolcanoesPosts(data);
      } catch (error) {
        console.error("Error fetching Volcanoes posts:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <p className="text-lg font-medium">Loading Volcanoes posts...</p>
      </div>
    );
  }
